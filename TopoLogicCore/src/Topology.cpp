...
namespace TopoLogicCore
{
	...
	void Topology::TestImposeIntermediate(const std::list<Topology*>& rkTopologyArguments, const std::list<Topology*>& rkTopologyTools, const bool kOutputCellComplex, std::list<Topology*>& rTopology)
	{
		BOPCol_ListOfShape occtShapeArguments;
		for (std::list<Topology*>::const_iterator kTopologyIterator = rkTopologyArguments.begin();
			kTopologyIterator != rkTopologyArguments.end();
			...
			kArgumentIterator++)
		{
			if (!occtMergeArguments.Contains(*kArgumentIterator))
			{
					...
					kArgumentImageIterator++)
				{
					...
					// TODO: Check the individual faces, later add them to create a shell
					bool hasElement = false;
					TopoDS_Builder occtBuilder;
					TopoDS_Shell occtImageShell;
					occtBuilder.MakeShell(occtImageShell);
						...
						kOcctArgumentImageFaceIterator++)
					{
							...
							kToolIterator++)
						{
								...
								kToolImageIterator++)
							{
								...
								//if (occtToolFaces.Contains(rkImageFaceShape))
								for (TopTools_MapOfShape::const_iterator kOcctToolImageFaceIterator = occtToolImageFaces.cbegin();
									kOcctToolImageFaceIterator != occtToolImageFaces.cend() && !isInTools;
									kOcctToolImageFaceIterator++)
								{
						...
						if (!isInTools)
						{
							...
							// This shows the faces which are NOT part of the tools.
							//rTopology.push_back(ByOcctShape(rkArgumentImageFace));
						}
					}
					...
					if(hasElement) 
					{
						...
						// This shows the shells only from the argument images
						rTopology.push_back(ByOcctShape(occtImageShell));
					}
				}
			}
		}
		...
		if (occtMergeArguments.Size() == 1)
		{
			if (kOutputCellComplex)
			{
				//rTopology.push_back(ByOcctShape(MakeCompSolid(*occtMergeArguments.begin())));
			}
			else
			{

				//rTopology.push_back(ByOcctShape(*occtMergeArguments.begin()));
			}
		}else
		{
			BOPAlgo_Builder occtAlgoBuilder;
			...
			if (kOutputCellComplex)
			{
				//rTopology.push_back(ByOcctShape(MakeCompSolid(rkOcctMergeShape)));
			}
			else
			{
				//rTopology.push_back(ByOcctShape(rkOcctMergeShape));
			}
		}
	}
